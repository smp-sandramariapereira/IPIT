...
```mermaid
flowchart TD
    P[Problema real] --> I[Investigação]
    I --> S[Ideia selecionada]
    S --> T[Tecnologia justificada]
    T --> M[MVP definido]
    M --> D[Desenvolvimento e testes]
    D --> E[Evidências no GitHub]
...
```mermaid
flowchart LR
    A[⚡ Micro-Ideathon] --> B[🧭 Jornada de 8 etapas]
    B --> C[📅 Projeto bimestral ou trimestral]
    C --> D[🏫 Programa institucional]
```
